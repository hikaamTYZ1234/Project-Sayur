...
class _HomeScreenState extends State<HomeScreen> with RouteAware {
  ...
  // Dipanggil saat kembali ke screen ini (misal: back dari food detail)
  @override
  void didPopNext() {
    _loadData();
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      final products = await ApiService.getTopRated();
      if (mounted) setState(() => _topRated = products);
    } catch (_) {
    } finally {
  ...
  Widget _buildTopRatedMenus(BuildContext context, bool isDark) {
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          final product = _topRated[index];
          final imageUrl = (product['image_url'] as String?)?.isNotEmpty == true
            ...
            onTap: () {
              ...
              Navigator.pushNamed(
                ...
                arguments: {
                  'id': product['id'],
                  'title': product['name'] ?? '',
                  'ingredients': product['category']?['name'] ?? '',
                  'price': '\$${product['price']}',
                  'image': imageUrl ?? '',
                },
              );
